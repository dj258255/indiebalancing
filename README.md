...
대부분의 인디 개발자가 엑셀/구글 시트를 사용하지만:

| 현상 | 왜 문제인가 |
|------|-------------|
| 게임 수식이 없음 | `DAMAGE(atk, def)`, `SCALE(base, level, rate, "exponential")` 같은 함수가 없어서 VLOOKUP과 복잡한 셀 참조로 구현해야 함. 수식이 길어지고 오류 발생 시 디버깅이 어려움 |
| 시트 간 연동이 수동 | 캐릭터 시트에서 ATK를 수정해도 스킬 데미지 시트에 자동 반영되지 않음. 매번 수동으로 값을 맞춰야 해서 불일치 발생 |
| 게임 특화 시각화 없음 | 성장 곡선(Linear/Exponential/S-Curve) 비교, TTK 분포 차트 등을 보려면 별도 도구가 필요함 |
| JSON 내보내기 불편 | 게임 엔진에서 쓸 JSON을 뽑으려면 Apps Script나 Python 스크립트를 따로 작성해야 함 |
...
- 스프레드시트의 한계는 "게임 도메인 지식"이 없다는 점

### 해결 방법

**어떻게 해결했는가:**

1. **게임 특화 수식 함수 23개 구현**
...
- **범용 전투 시뮬레이터 아님**: 전투 로직은 게임마다 다름. 수식 기반 계산만 제공
- **게임 엔진 플러그인 아님**: JSON 데이터 제공. 파싱은 개발자가 직접
...
A: 없습니다. 게임마다 전투 로직이 다르기 때문에 범용 시뮬레이터는 현실적으로 어렵습니다. 대신 TTK/DPS 계산기를 제공합니다.
...
A: 지원하지 않습니다. 프로젝트마다 구조가 달라서 어차피 수정이 필요합니다. JSON 파싱은 개발자가 직접 하시면 됩니다.
...
A: 엑셀에는 `SCALE(base, level, rate, "exponential")` 같은 게임 특화 함수가 없습니다. 순환 참조 감지 경고나 원클릭 JSON 내보내기도 마찬가지입니다. 경쟁 서비스 분석 섹션을 참고해주세요.
...
A: 정적 호스팅이라 서버 비용이 거의 없습니다. Vercel 무료 티어로 충분하고, 광고 1~2개로 도메인 비용 정도는 충당 가능합니다.
